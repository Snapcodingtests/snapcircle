...
function drawFrame(frameBytes) {
  const imageData = new ImageData(new Uint8ClampedArray(frameBytes), 160, 144);
...
startBtn.addEventListener('click', () => {
  if (!hasRom) {
    setStatus('Please choose a .gb ROM before starting.', 'error');
    return;
  }
  if (!running) {
    running = true;
...
window.addEventListener('keydown', (event) => {
  ...
  pyodide.globals.set('btn_name', key);
  pyodide.runPython('import gb_emulator\ngb_emulator.set_button(btn_name, True)');
});
...
bootPython().catch((err) => {
  console.error(err);
  setStatus(`Runtime init failed: ${err.message}`, 'error');
});
